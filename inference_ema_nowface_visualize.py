...
def main(
    ...
    hd_renderer = HDRenderer(img_size=head_crop_size)
    ...
    for idx, batch in enumerate(tqdm(head_dloader)):
        ...
        # add for rendering visualization pictures
        hd_imgs = head_imgs.images.detach().cpu().numpy().squeeze()
        _, _, H, W = head_imgs.shape
        ...
        for idx in tqdm(range(len(batch_size)), 'Saving ...'):
            img_path = head_targets[idx].get_field('fname')#data/now/NoW_Dataset/final_release_version/iphone_pictures/FaMoS_*/multiview_neutral/IMG_*.jpg
            ...
            fname=subject_name+'-'+challenge+'-'+img_name
