        ...
        for dataloader in dataloaders:
            ...
            for idx, batch in enumerate(
                ...
                if full_imgs is not None:
                    full_imgs = full_imgs.as_tensor().to(device=device)
                ...
                #     full_imgs=full_imgs,
                #     device=device)
                square_full_imgs=full_imgs[:,:,:,200:1400]
                batch={'image':body_imgs, 'image_hd':square_full_imgs}#body_imgs} #full_imgs}
                data = {
                    'body': batch
                }
                param_dict = model.encode(data, threthold=True, keep_local=True, copy_and_paste=False)
                codedict = param_dict['body']
                opdict = model.decode(codedict, param_type='body')
                #opdict keys: ['vertices'],['transformed_vertices'],['face_kpt'],
                #['smplx_kpt'],#2d joints
                #['smplx_kpt3d'],#the same as joints
                #['joints'],['cam'],
                mydict={'vertices':opdict['vertices'],
                        'joints':opdict['joints'],
                        'betas':codedict['shape'][:,:10],
                        'expression':codedict['exp'][:,:10],
                        'body_pose':codedict['body_pose'],
                        'global_orient':codedict['global_pose'],
                        'right_hand_pose':codedict['right_hand_pose'],
                        'left_hand_pose':codedict['left_hand_pose'],
                        'jaw_pose':codedict['jaw_pose'],
                        }
                '''vertices shape: torch.Size([16, 10475, 3])
                joints shape: torch.Size([16, 145, 3])
                betas shape: torch.Size([16, 200])# should be [16,10]
                expression shape: torch.Size([16, 50])# should be [16,10]
                body_pose shape: torch.Size([16, 21, 3, 3])
                global_orient shape: torch.Size([16, 1, 3, 3])
                right_hand_pose shape: torch.Size([16, 15, 3, 3])
                left_hand_pose shape: torch.Size([16, 15, 3, 3])
                jaw_pose shape: torch.Size([16, 1, 3, 3])
                camera_parameters shape: torch.Size([16, 3])'''

                body_output={'final':mydict, 'camera_parameters':opdict['cam']}
