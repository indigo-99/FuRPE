...
class EHF(dutils.Dataset):
    ...
    def __init__(self, data_folder, img_folder='images',
        ...
        hand_ehf_fns="/data/panyuqing/expose_experts/data/curated_fits/hand_EHF_comp_img_fns.npy"  
        if False:#osp.exists(hand_ehf_fns):
            self.img_fns=list(np.load(hand_ehf_fns, allow_pickle=True))
        else:
            npy_fns = sorted(os.listdir('/data/panyuqing/expose_experts/data/params3d_v/EHF'))
            self.img_fns =['/data/panyuqing/expose_experts/data/EHF/images/'+npf[:-4]+'.png' for npf in npy_fns]
            self.annotations=[npf[:-4] for npf in npy_fns]
    ...
    def __getitem__(self, index):
        ...
        #fn:66_img -> keypoint_fn:66_2Djnt.json
        ...
        # p add for test the pseudo tags
        pseudo_fn=osp.join('/data/panyuqing/expose_experts/data/params3d_v/EHF',fn+'.npy')
        pseudo_params=np.load(pseudo_fn, allow_pickle=True).item()
        #target.add_field('pseudo_vertices', pseudo_params['vertices'])
        #target.add_field('pseudo_joints', pseudo_params['keypoints3d'])
        target.add_field('pseudo_vertices', pseudo_params['vertices_noshape'])
        target.add_field('pseudo_joints', pseudo_params['keypoints3d_noshape'])
