...
# add visualization code to render ground truth vertices from paramter labels and save them for faster evaluation
...
# Joint selectors: Indices to get the 14 LSP joints from the 17 H36M joints
...
class ThreeDPW(dutils.Dataset):
    '''
    The class of the dataloader designed for the 3DPW evaluation dataset.
    '''
    def __init__(self, data_path='data/3dpw',
        ...
        # set the data path (only use testing)
        if self.split == 'train':
        ...
        # the person id to distinguish which person he is, because an image may has 2 people
        ...
        # extract pose and betas from labels to build the ground truth vertices
    ...
    def __getitem__(self, index):
        ...
        if self.return_vertices: #smplx_vertices/
            ...
            # if vertices do not exist, generate them by feeding ground truth parameters into the SMPL model 
            # only being executed for the first-time 3dpw evaluation
            if osp.exists(vertex_fname):
                ...
                o3d.io.write_triangle_mesh(vertex_fname, open3dmesh) # save .ply vertice file
