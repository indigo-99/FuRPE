...
class MANOLossModule(nn.Module):
    ...
    def forward(self, input_dict,
        ...
        global_orient = torch.stack(
            [t.get_field('global_pose').global_pose for t in hand_targets
             if t.has_field('global_pose')])
        ...
        if compute_2d_loss:
            ...
            hand_joints2d_loss = self.joints_2d_loss(
                hand_proj_joints,
                hand_keyps[:, self.right_hand_idxs], #gt kpt2d shape: 127+17,3
        ...
        if compute_3d_joint_loss:
            ...
            # target is transformed to shape(127+17,3), so necessary to extract hand_idxs
            # but predicted 2/3d keypoints only contain (48,3) hand kpts, so no extraction need
        ...
        for n in range(1, num_stages + 1):
            ...
            curr_losses = self.single_loss_step(
                curr_params,
                hand_pose=gt_hand_pose,#can add shape=... if needs shape_loss
