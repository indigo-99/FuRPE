    ...
    def run_body_eval(self, dataloaders, model, step, alignments=None,
        ...
        for dataloader in dataloaders:
            ...
            for idx, batch in enumerate(
                ...
                body_vertices =np.array([target.get_field('pseudo_vertices') for target in body_targets])
                body_joints = np.array([target.get_field('pseudo_joints') for target in body_targets])
                ...
                if compute_mpjpe14 and body_vertices is not None:
                    ...
                    if len(gt_vertices) > 0:
                        ...
                        #logger.info('body_vertices shape: {}',body_vertices.shape)
                        #ehf: (32, 10475, 3)
                        ...
                        #logger.info('gt_joints14[0]: {}',gt_joints14[0])
                        #logger.info('gt_joints14_reg[0]: {}',gt_joints14_reg[0])
                        #p add
                        for alignment_name, alignment in alignments.items():
