...
class SMPLXHead_online(nn.Module):
    ...
    def __init__(
        ...
        model_path = osp.expandvars(body_model_cfg.get('model_folder', ''))
    ...
    def forward(self,
        ...
        for key in deltas_dict:#not be used in the following codes?
        ...
        if self.predict_head:
            if self.apply_head_network_on_body:
                ...
                )#factor:1.5, crop_size=image.shape[2]=width
                ...
                #logger.info('head_bbox_size: {}',head_bbox_size)
        ...
        else: #test do not return losses and feats
            output = {
                'body': out_params,
            }
