    ...
    def __call__(self, vertices, faces,
        ...
        for bidx in range(batch_size):
            ...
            if len(color) < 4:
                s = list(color.shape)
                s[0] = 1
                new_axis_arr = np.ones(s) #(1,256,256)'s 1
                color = np.concatenate([color, new_axis_arr], axis=0)#(3,256,256)->(4,256,256)
            ...
            if bg_imgs is None:
                ...
                if return_with_alpha:
                    ...
                    #output_img = (color[:, :, :3] * valid_mask + (1 - valid_mask) * curr_bg_img)
                    output_img = (color * valid_mask + (1 - valid_mask) * curr_bg_img)
                    output_imgs.append(np.clip(output_img, 0, 1))
                else:
                    #valid_mask = (color[3] > 0)[np.newaxis] #shape: (1,256,256)
                    valid_mask = (depth > 0)[np.newaxis]#[:,:,None]
                    #print('valid_mask: ',valid_mask.shape)
                    #print('color[:-1]: ',color[:-1].shape)
                    #print('bg_imgs[bidx]: ',bg_imgs[bidx].shape)
                    #output_img = (color[:, :, :3] * valid_mask + (1 - valid_mask) * curr_bg_img)
...
class GTRenderer(AbstractRenderer):
    ...
    def create_mesh(self, vertices, faces, color=(0.3, 0.3, 0.3, 1.0),
        ...
        o3d.io.write_triangle_mesh('/data/panyuqing/expose_experts/gtmesh.ply', open3dmesh)
        return self.trimesh_to_pymesh(mesh, material=material)
    ...
    def __call__(self, vertices, faces,
        ...
        for bidx in range(batch_size):
            ...
            if bg_imgs is None:
                ...
                if return_with_alpha:
                    #valid_mask = (color[3] > 0)[np.newaxis]
                    valid_mask = (depth > 0)[np.newaxis]#p add
