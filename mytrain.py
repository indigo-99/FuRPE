...
class ExTrainer(object):
    ...
    def __init__(self, exp_cfg):
        ...
        self.optimizer = build_optimizer(model=self.model,optim_cfg=optim_cfg)
    ...
    def train_step(self, input_batch):
        ...
        # add for check error in training_distil_experts
        '''
        for i in range(4):
            # target_i=cropped_targets[i]
            # bpi=target_i.get_field('body_pose').body_pose
            # bti=target_i.get_field('betas')
            # logger.info('body_pose_{}: {}',i,bpi)
            # logger.info('betas_{}: {}',i,bti)
            img_i=images[i].detach().cpu().numpy()
            bd_img_i=cropped_images[i].detach().cpu().numpy()
            
            img_c = img_i.transpose((1, 2, 0))
            bd_img_c = bd_img_i.transpose((1, 2, 0))
            #logger.info('img_i shape: {}',img_c.shape)
            #logger.info('bd_img_i shape: {}',bd_img_c.shape)
            cv2.imwrite('./input_image_'+str(i)+'.png',img_c*255)
            cv2.imwrite('./input_crop_image_'+str(i)+'.png',bd_img_c*255)
            # logger.info('img_{}: {}',i,img_i)
            # logger.info('bd_img_{}: {}',i,bd_img_i)
        '''
        ...
        if not freeze_body:
            loss+=loss_bd_pose
            loss += out_losses['keypoint_loss']['body_joints_2d_loss']
  
        if 'global_orient_loss' in out_losses['body_loss']:
            loss_global_orient=out_losses['body_loss']['global_orient_loss']
            loss += loss_global_orient
        
        if not freeze_hand:
            hand_joints_2d_loss=2*out_losses['keypoint_loss']['hand_joints_2d_loss']
            loss += hand_joints_2d_loss
        
        if not freeze_face:
            loss += out_losses['keypoint_loss']['face_joints_2d_loss']
            if ('expression_loss' in out_losses['body_loss']):
                loss_expression=out_losses['body_loss']['expression_loss']
                loss+=loss_expression
            if 'jaw_pose_loss' in out_losses['body_loss']:
                loss_jaw_pose=out_losses['body_loss']['jaw_pose_loss']
                loss+=loss_jaw_pose
                if use_face_feature_distil:
                    ...
                    feature_face_loss=self.klloss(pred_feat,gt_feat)*100000
        ...
        if not freeze_hand:
            ...
            if 'left_hand_pose_loss' in out_losses['body_loss']:
                loss_lh_pose=10*out_losses['body_loss']['left_hand_pose_loss']
                loss+=loss_lh_pose
                if use_hand_feature_distil and loss_lh_pose!=0:
                    ...
                    if len(has_feature_left_hand_idxes)>0:
                        ...
                        feature_left_hand_loss=self.klloss(pred_left_hand,gt_feat)*100000
                        loss += feature_left_hand_loss

            feature_right_hand_loss=None
            if 'right_hand_pose_loss' in out_losses['body_loss']:
                loss_rh_pose=10*out_losses['body_loss']['right_hand_pose_loss']
                loss+=loss_rh_pose
                if use_hand_feature_distil and loss_rh_pose!=0:
                    ...
                    if len(has_feature_right_hand_idxes)>0:
                        ...
                        feature_right_hand_loss=self.klloss(pred_right_hand,gt_feat)*100000
                        loss += feature_right_hand_loss

        # for losskey in out_losses['keypoint_loss']:
        #     loss += out_losses['keypoint_loss'][losskey]# add 3d joints loss if in key
        ...
        if use_body_feature_distil and (not freeze_body):
            ...
            feature_body_loss=self.klloss(pred_feat,gt_feat)*100000
        ...
        if not freeze_body:
            losses['body_joints_2d_loss']=out_losses['keypoint_loss']['body_joints_2d_loss'].detach().item()
        ...
        if not freeze_face:
            losses['face_joints_2d_loss']=out_losses['keypoint_loss']['face_joints_2d_loss'].detach().item()
            if 'jaw_pose_loss' in out_losses['body_loss']:
